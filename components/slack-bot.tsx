...
import { Message } from "@/lib/domain/models/bot-message";
import { GetBotResponseUseCase } from "@/lib/application/use-cases/get-bot";
import { StaticBotRepository } from "@/lib/infrastructure/repositories/static-bot";

// Clean Architecture: Dependency Injection / Instantiation
const botRepository = new StaticBotRepository();
const getBotResponseUseCase = new GetBotResponseUseCase(botRepository);
...
export function SlackBotSection() {
    ...
    const handleSend = async () => {
        ...
        const response = await getBotResponseUseCase.execute(userText);
